...
bool Heeps::HandleIncomingPacket(uint16_t id, uint32_t size, const uint8_t* data, uint8_t* modified, uint32_t sizeChunk, const uint8_t* dataChunk, bool injected, bool blocked)
{
    ...
    bool isPet = (entityInfo->ownerid != 0);
        ...
        else
        {
            switch (actionType)
            {
                ...
                case ACTIONTYPE_NPC_TP_FINISH:
                {
                    ...
                    auto ability = m_AshitaCore->GetResourceManager()->GetAbilityById(actionID);
                    if (ability != nullptr && ability->Name[2] != nullptr)
                        newsource.name.append(ability->Name[2]);
                    else
                        newsource.name.append("Unknown WS");
                    break;
                }
                case ACTIONTYPE_CAST_FINISH:
                {
                    ...
                    auto spell = m_AshitaCore->GetResourceManager()->GetSpellById(actionID);
                    if (spell != nullptr && spell->Name[2] != nullptr)
                        newsource.name.append(spell->Name[2]);
                    else
                        newsource.name.append("Unknown Spell");
                ...
                case ACTIONTYPE_ITEM_FINISH:
                {
                    ...
                    auto item = m_AshitaCore->GetResourceManager()->GetItemById(actionID);
                    if (item != nullptr && item->Name[0] != nullptr)
                        newsource.name.append(item->Name[0]);
                    else
                        newsource.name.append("Unknown Item");
                    break;
                }
                case ACTIONTYPE_JA:
                ...
                case ACTIONTYPE_JA_RUN:
                {
                    ...
                    auto ability = m_AshitaCore->GetResourceManager()->GetAbilityById(actionID + 512);
                    if (ability != nullptr && ability->Name[2] != nullptr)
                        newsource.name.append(ability->Name[2]);
                    else
                        newsource.name.append("Unknown Ability");
                    break;
                }
                case ACTIONTYPE_AVATAR_BP_FINISH:
                {
                     ...
                     if (ability != nullptr && ability->Name[2] != nullptr)
                        ...
                        newsource.name.append("Unknown BP");
                     break;
                }
                default:
